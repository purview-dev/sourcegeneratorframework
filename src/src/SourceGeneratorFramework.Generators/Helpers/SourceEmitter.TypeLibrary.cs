...
partial class SourceEmitter
{
				...
				bodyWriter =>
				{
					bodyWriter
						...
						.Constructor(
							...
							constructorWriter =>
								constructorWriter.Assignment("Namespace", "@namespace").Assignment("Arity", "arity")
						);
	...
	public static SourceText EmitTypeLibrary(TypeLibraryModel model)
	{
		...
		writer.AutoGeneratedHeader().Using("global::Purview.SourceGeneratorFramework").NewLine();

		if (string.IsNullOrEmpty(model.Namespace))
		{
			TypeLibraryClass(writer, model);
		}
		else
		{
	...
	static void TypeLibraryClass(CodeWriter writer, TypeLibraryModel model)
	{
		WriteDocumentation(writer, model.Documentation);
	...
	static void GetTypesMethod(CodeWriter writer, TypeLibraryNamespaceNode node)
	{
		...
		writer.MethodExpression(
			new MethodDeclarationOptions("GetTypes", ImmutableArrayOfTypeReference, TypeDeclarationAccessibility.Public)
			{
				IsStatic = true,
	...
	{
		WriteDocumentation(writer, member.Documentation);
	...
	static void WriteDocumentation(CodeWriter writer, string? documentation)
	{
		...
		writer.XmlComment(documentation!.Replace("\r\n", "\n").Split('\n'));
	}
	...
	static TypeReference ImmutableArrayOfTypeReference =>
		new TypeIdentity("ImmutableArray", "System.Collections.Immutable", 1)
			.MakeGeneric(GeneratorTypeLibrary.TypeReferenceValueObject)
			.AsTypeReference();
	...
	public static SourceText EmitTypeRefUsagePartial(TypeLibraryModel model)
	{
		...
		if (string.IsNullOrEmpty(model.SpecNamespace))
		{
			TypeRefUsagePartial(writer, model);
		}
		else
		{
			using (writer.BlockNamespaceScope(model.SpecNamespace))
				TypeRefUsagePartial(writer, model);
		}
	...
	static void TypeRefUsagePartial(CodeWriter writer, TypeLibraryModel model)
	{
		using (
			writer.ClassScope(
		...
		)
		{
			WriteDocumentation(
				writer,
				"The [TypeRef] marker identities declared by this spec. The source generator reads them at compile time; this member keeps them visible to the compiler's unused-member analysis."
			);
